...
pub fn save_to_pc(url: &str) -> anyhow::Result<()> {
    ...
    //the full path to the archive (ie. $HOME/.local/share/kyro/engnet_usfx.zip)
    let file_zip_path: PathBuf = data_dir.join("engnet_usfx.zip");

    //if the directory doesn't exist then lets create it
    if !unzip_dir.is_dir() {
        fs::create_dir_all(&unzip_dir).context("could not create kyro project data directory")?;
    }
    ...
    if !file_path.is_file() {
        ...
        unzip(&file_zip_path, &unzip_dir).context("unzipping has failed some how")?;
    }
    Ok(())
}

fn unzip(zipped_file_path: &PathBuf, dest_dir: &PathBuf) -> anyhow::Result<()> {
